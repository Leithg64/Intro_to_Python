...
class RecipeViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.recipe1 = Recipe.objects.create(
            name="Recipe 1", ingredients="ingredient1, ingredient2", cooking_time=10
        )
        cls.recipe2 = Recipe.objects.create(
            name="Recipe 2", ingredients="ingredient1, ingredient2", cooking_time=20
        )
    ...
    def test_search_view_login_required(self):
        ...
        # check if response redirects to login page with the next parameter set to requested URL
        self.assertRedirects(
            response, f"{reverse('login')}?next={reverse('recipes:search')}"
        )
...
class AddRecipeViewTest(TestCase):
    ...
    def test_add_recipe_view_login_required(self):
        ...
        self.assertRedirects(
            response, f"{reverse('login')}?next={reverse('recipes:add_recipe')}"
        )